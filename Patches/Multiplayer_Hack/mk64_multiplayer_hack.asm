...
scope Init: {
  ...
  Menu:
    ...
    Default:
      la t0, Options
      ori t1, r0, 0x02
      sb t1, 2 (t0) // Scaling Fix
      sb t1, 3 (t0) // Emulator Lag Fix
      sb t1, 6 (t0) // Trophies
...
scope LagFix: { // Available registers: t9, t2
  LuiLb(t9, Options+3)
  Disabled:
    OriBne(t9, 0x01, t2, Enabled) // If option disabled
    lw t6, 0x0010(s0) // Original instruction
    b End
    nop
  Enabled:
    OriBne(t9, 0x02, t2, End) // If option enabled
    lw t6, 0x0010(s0) // Original instruction
    li t9, 0xFFFF00FF
    and t6, t6, t9
    ori t6, t6, 0x0D00
  End:
    jr ra
    sw t6, 0x0018(t8) // Original instruction
}
